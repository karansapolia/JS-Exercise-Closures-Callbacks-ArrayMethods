...
function processSum(numberList, callback) {
  let sum = numberList.reduce((sum, currentVal) => sum + currentVal, 0);
  return callback(sum);
}
...
function processDuplicateFree(list, callback) {
  const set = list.filter((value, index) => value !== list[index - 1]);
  return callback(set);
}
...
function getFullNames(runners) {
  ...
  runners.forEach((person) => {
    fullNames.push(`${person.last_name}, ${person.first_name}`);
  });
  return fullNames;
}
...
function firstNamesAllCaps(runners) {
  const firstNames = runners.map((person) => person.first_name.toUpperCase());
  return firstNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const peopleByShirtSizes = runners.filter((person) => person.shirt_size === tShirtSize);
  return peopleByShirtSizes;
}
...
function tallyUpDonations(runners) {
  return runners.reduce((tally, person) => tally += person.donation, 0);
}
 ...
 * etc
*/
function counterMakerWithLimit(maxValue = 3) {
  let counter = 0;
  return function() {
